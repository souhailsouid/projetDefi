...
export const useGetATokenDataAndBalance = () => {
  const { address } = useAccount();
  const [iATokenDataAndBalanceLoading, setIsLoading] = useState(true);
  const [errorATokenDataAndBalance, setError] = useState(null);
  const [getATokenDataAndBalance, setData] = useState(null);
  ...
  const contractCalls = reserveListData.map((asset) => ({
    ...
    abi: ContractViewAbi,
    functionName: 'getATokenDataAndBalance',
    args: [asset, address],
  }));
  ...
  const result = data?.map((item) => {
    const [tokenData, balance, additionalData] = item.result;
    // Example of displaying the data
    return {
      symbol: tokenData.symbol.split('aEth')[1],
  ...
  return {
    getATokenDataAndBalance,
    errorATokenDataAndBalance,
    iATokenDataAndBalanceLoading,
  };
};
