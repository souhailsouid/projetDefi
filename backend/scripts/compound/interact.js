...
describe("CompoundInteraction", function () {
  it("Should supply and withdraw USDC dynamically", async function () {
    ...
    await compoundInteraction.supply(cTokenAddress, underlyingTokenAddress, 1000);
    console.log("Supply transaction completed");

    // Check cToken balance after supply
    const cTokenBalance = await USDC.balanceOf(compoundInteraction.target);
    console.log("cToken Balance after supply:", cTokenBalance.toString());

    // Withdraw USDC from the CompoundInteraction contract
    await compoundInteraction.withdraw(cTokenAddress, underlyingTokenAddress, 1000);
